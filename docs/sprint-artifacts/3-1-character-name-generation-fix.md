...
- [x] 在prompt_definitions.py中查找角色生成提示词模板
- [x] 审查当前提示词的结构和参数
- [x] 优化提示词使其明确要求合理名字格式
- [x] 添加角色特征参数(性别、年龄、文化背景)
- [x] 测试不同提示词版本的名字生成质量
- [x] 选择最优提示词模板

### 任务2: 名字验证逻辑实现 (AC#1, #4)
- [x] 在novel_generator/chapter.py中创建validate_character_name()函数
- [x] 实现字符有效性检查(正则表达式: ^[\u4e00-\u9fa5a-zA-Z\s]+$)
- [x] 实现长度验证(最小2字符,最大20字符)
- [x] 实现乱码检测(检查Unicode范围)
- [x] 实现不合理字符过滤(数字、特殊符号等)
- [x] 添加测试用例验证验证逻辑

### 任务3: 角色特征一致性检查 (AC#2)
- [x] 创建check_name_consistency(name, traits)函数
- [x] 实现性别特征一致性检查(如男性名字不包含'娜'、'丽'等女性化字)
- [x] 实现文化背景一致性检查(如东方角色有东方风格名字)
- [x] 实现名字年龄一致性检查(如老年角色名字不含有网络流行语)
- [x] 如果不一致,触发重新生成或记录警告日志

### 任务4: 跨章节名字一致性保证 (AC#3)
- [x] 创建character_name_registry字典,保存角色ID到名字的映射
- [x] 在生成角色时检查registry中是否已有该角色
- [x] 如果已有,使用registry中保存的名字
- [x] 如果是新角色,生成名字并保存到registry
- [x] 在project save/load时持久化和恢复registry
- [x] 测试多章节名字一致性

### 任务5: 名字格式标准化 (AC#5)
- [x] 创建normalize_character_name(name)函数
- [x] 实现去除前后空白
- [x] 实现去除多余内部空格
- [x] 实现首字母大写或全名适当格式化
- [x] 在保存和使用名字前调用标准化
...
- [x] 在名字生成失败时捕获异常
- [x] 实现最多3次重试机制
- [x] 如果3次都失败,返回默认名字"待定角色"并记录警告
- [x] 在状态栏显示"角色名字生成失败,使用默认名字"消息
- [x] 在app.log中记录详细错误信息

### 任务7: 集成和端到端测试
- [x] 在generate_character()或类似函数中集成名字验证
- [x] 测试完整角色生成流程
- [x] 生成多章节小说验证名字一致性
- [x] 测试各种边界情况(特殊字符、极长名字等)
- [x] 验证与Epic 1配置系统的集成
...
**实现完成总结:**
✅ **优化了AI角色生成提示词** - 添加了详细的名字生成规范和验证机制
✅ **实现了完整的名字验证逻辑** - 包含长度、字符有效性、性别一致性检查
✅ **建立了跨章节名字一致性保证** - 通过注册表机制确保角色名字统一
✅ **集成了错误处理和重试机制** - 最多3次重试，失败时使用默认名字
✅ **添加了端到端测试验证** - 确保所有功能正常工作

**技术改进:**
- 提示词明确要求2-4字符中文名字，避免乱码和特殊符号
- 实现性别一致性检查，男性角色避免女性化字词，反之亦然
- 建立角色名字注册表，支持项目持久化
- 在UI角色管理器中集成验证逻辑，实时检查生成结果

### File List

**修改的文件:**
- `prompt_definitions.py` - 优化了ai_role_generation_prompt提示词，添加了详细的名字生成规范
- `novel_generator/chapter.py` - 添加了完整的名字验证和管理功能（validate_character_name, normalize_character_name等）
- `ui_qt/widgets/role_manager.py` - 集成了名字验证逻辑和重试机制

**新增的测试文件:**
- `test_character_name_validation.py` - 完整的功能测试套件
- `test_name_validation_simple.py` - 简化的核心功能测试
